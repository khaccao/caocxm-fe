...
// [27/11] [ngoc_td]: Tính tổng số lượng sản phẩm đã nhập kho của phiếu
...
export interface ImportGood {
  madvcs: string;
  ma_ct: string;
  ngay_ct: string; // ISO 8601 date string
  so_ct: string;
  ma_kh: string;
  nguoi_tt: string;
  dien_giai: string;
...
const ImportGoods: React.FC<{ proposal: ProposalData; handleClose: () => void, type: eTypeVatTuMayMoc }> = ({ proposal, handleClose, type }) => {
  ...
  //[10/1/2025][ngoc_td] xác định phiếu đã duyệt chi tiền chưa
  const hasSuffix = (soCt: string) => /\.\d{3}$/.test(soCt);
  const [isApproved, setIsApproved] = useState(false);
  useEffect(() => {
    if (hasSuffix(proposal.so_ct)) {
      setIsApproved(true);
    } else {
      setIsApproved(false);
    }
  ...
  const onSaveWarehouseReceipt = async () => {
    try {
      ...
      // Lọc các item có cả xacNhanHangHoa1 và xacNhanHangHoa2 đều rỗng
      const updatedChiTietHangHoa = chiTietHangHoa
        .map(item => {
        ...
        .filter(item => item.xacNhanHangHoa1 !== '' || item.xacNhanHangHoa2 !== ''); // Loại bỏ các item có cả hai trường rỗng
      const updateThueVAT = chiTietHangHoa.every(item => item.vatRate === 0)
        ...
        : chiTietHangHoa.map((item) => {
          ...
          return {
            ...
            "guidRelation": guid,
            "tk_no": '1331',
            "tk_co": productList.find(product => product.ma_vt === item.ma_vt)?.productType === 0 ? '3313' : '3311',
            "chiTietHangHoaVAT": [
              {
                "id": 0,
                "guid": "00000000-0000-0000-0000-000000000000",
  ...
  useEffect(() => {
    setTongGiaTri(chiTietHangHoa.reduce((sum, item) => {
      ...
      const price = item.gia ?? 0;
      const tax = item.vatRate === -1 ? 0 : item.vatRate || 0;
  ...
  useEffect(() => {
    ...
    const updatedData = dataMuaHang.map((item: any) => {
      ...
      const totalQuantity = danhSachSoLuong.find(ds => ds.ma_vt === item.ma_vt)?.tong_so_luong || 0; // Default to 0 if not found
  ...
  const columns: ColumnsType<any> = [
    ...
    },
    {
      ...
      render: (_: any, record: any) => {
        ...
        const disabled = !approveChtGranted || selectedProposal.daChiTien !== 1;
    ...
    },
    {
      ...
      render: (_: any, record: any) => {
        ...
        const tooltipTitle = selectedProposal.daChiTien !== 1 ? 'Phiếu chưa được Duyệt chi tiền không hỗ trợ Xác nhận nhập kho' : undefined;
        return (
          <Tooltip title={tooltipTitle}>
            <span style={{ display: 'inline-block' }}>
              <Button
  ...
  return (
    <div>
      <div id="div1">
        <Row>
          <Col span={24}>
            <Title level={4}>{t('Import warehouse goods')}</Title>
          </Col>
        </Row>
        <Row >
          {/* <span style={{ width: '34%' }}>
