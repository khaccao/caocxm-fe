...
export default class ClassicEditor extends ClassicEditorBase {
  ...
  customParam: ICustomParam = {auth: null, company: null, user: null, itemId: 0};  
  setCustomParam(customParam: ICustomParam) {
    this.customParam = customParam;  
  }
}

function CkCustomUploadAdapterPlugin( editor: ClassicEditor ) {
  editor.plugins.get( 'FileRepository' ).createUploadAdapter = ( loader: any ) => {
      // Configure the URL to the upload script in your backend here!
      return new CkUploadAdapter( loader,  editor.customParam, apiUrl);
  };
}
...
ClassicEditor.defaultConfig = {
  auth: '',
  ckfinder: {
  	uploadUrl: `https://ncts.hicas.vn/api/photo/upload-ckeditor`
  },
  toolbar: {
