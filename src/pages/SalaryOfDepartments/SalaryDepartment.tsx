...
export const SalaryOfDepartment = () => {
  ...
  useEffect(() => {
    if (listDataFileRoots && listDataFileRoots?.results?.length > 0) {
      const rootId = listDataFileRoots.results.find((i: any) => i.name === 'luongbophan');
      if (rootId) {
  ...
  return (
    <>
      ...
      <DocumentsTable
        pass={activeMenu}
        policies={{
          create: ['KPI.LuongBoPhan.Create'],
          delete: ['KPI.LuongBoPhan.Delete'],
        }}
      />
    </>
  );
