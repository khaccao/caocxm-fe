...
export const HolidayCost = () => {
  ...
  useEffect(() => {
    if (listDataFileRoots && listDataFileRoots?.results?.length > 0) {
      const rootId = listDataFileRoots.results.find((i: any) => i.name === 'chiphithuongletet');
      if (rootId) {
  ...
  return (
    <>
      ...
      <DocumentsTable
        pass={activeMenu}
        policies={{
          create: ['CongDoan.CPLeTet.Create'],
          delete: ['CongDoan.CPLeTet.Delete'],
        }}
      />
    </>
  );
